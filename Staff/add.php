  ...
  '" . mysqli_real_escape_string($connection, '0') . "')";
    ...
    if ($result2) {
      ...
      try {
          ...
          $mail->setFrom('thisarasadesh4@gmail.com', 'School Admin');
          $mail->addAddress($_POST['emailaddress']); // User's email
  
          // Email Content
          $mail->isHTML(true);
          $mail->Subject = "School Staff Registration Successful";
          $mail->Body    = "
              <h3>Dear {$_POST['txtstfname']},</h3>
              <p>Your registration as a school staff member has been successfully completed.</p>
              <p><strong>User ID:</strong> {$stfid}<br>
                 <strong>Password:</strong> {$_POST['txtnic']}</p>
              <p>Please log in and update your password for security reasons.</p>
              <p>Best Regards,<br>School Administration</p>";
  
          // Send the email
          $mail->send();
          echo "<script>alert('Successfully Inserted into Database. Confirmation email sent.');window.location.href='index.php';</script>";
  
      } catch (Exception $e) {
          echo "<script>alert('Successfully Inserted into Database. But email sending failed: {$mail->ErrorInfo}');window.location.href='index.php';</script>";
      }
  }
  }
?>
            ...
            <div class="panel-body">
              <div class="table-responsive">
                <table class="table table-striped table-bordered table-hover" width="200" border="0" align="center">
                  ...
                  <tr>
                    <td>Email Address</td>
                    <td><input name="emailaddress" type="email" required class="form-control" id="emailaddress" placeholder="Type Email Address"></td>
                  </tr>
                  <tr>
                    <td>Age</td>
